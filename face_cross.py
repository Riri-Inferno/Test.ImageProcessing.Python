...
def draw_cross_on_faces(input_image_filename, output_image_filename):
    ...
    if len(faces) > 0:
        ...
        print(f"{len(faces)} 個の顔にバツ印を描画しました。")
    else:
        print("顔が検出されませんでした。")
...
if __name__ == "__main__":
    # 入力画像と出力画像のファイル名を指定
    input_image_filename = 'input.jpg'   # 処理したい画像のファイル名
    output_image_filename = 'output.jpg'  # 処理後の画像のファイル名

    draw_cross_on_faces(input_image_filename, output_image_filename)